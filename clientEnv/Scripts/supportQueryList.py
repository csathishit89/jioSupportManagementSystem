def supportQueryList(st, updateQueryStatus, pd, clientQueriesListFetch, clientHistoryQueriesListFetch, categoryListFetch):
    
    ### To fetch category list - Start ###
    categoryListLoad = categoryListFetch.categoryListFetch()
    categoryList =  [(None, "Select category")] + categoryListLoad
    ### To fetch category list - End ###
    category_id=''
    
    ### To fetch client query list - Start ###
    clientsQueriesListFetch = clientQueriesListFetch.clientQueriesListFetch('All', category_id)
    data_records = clientsQueriesListFetch
    ### To fetch client query list - End ###
    
    ### To fetch client history query list - Start ###
    clientsHistoryQueriesListFetch = clientHistoryQueriesListFetch.clientHistoryQueriesListFetch(category_id)
    data_history_records = clientsHistoryQueriesListFetch
    ### To fetch client history query list - End ###
    ...
    col1, col2, col3, col4  = st.columns([1, 25, 15, 15])
    with col2:
    ...
    with col3:
        category = st.selectbox(
            "**Select Category**",
            categoryList,
            format_func=lambda x: x[1] if x[0] is not None else "Select category",
            index=0,
            key="category_select"
        )
    ...
    with col4:
        ...
        if selected_option == "All":
            clientsQueriesListFetch = clientQueriesListFetch.clientQueriesListFetch(selected_option, category_id)
            data_records = clientsQueriesListFetch
            df = pd.DataFrame(data_records, columns=column_names)
